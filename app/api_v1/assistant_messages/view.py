...
from .schemas import AssistantMessageCreate, AssistantMessageRead, AssistantMessageUpdate
...
router = APIRouter(tags=["AssistantMessages"], dependencies=[Depends(get_current_auth_user)])
...
@router.post("", response_model=AssistantMessageRead, status_code=status.HTTP_201_CREATED)
async def create_assistant_message(
    ...
    return await assistant_messages_service.add(session=session, values=assistant_message_in)
...
async def delete_assistant_message(
    ...
    return await assistant_messages_service.delete(session=session, filters=assistant_message)
