...
async def run(snapshot):
    ...
    async for session in session_manager.get_db():
        ...
        await snapshots_service.update(
            ...
            values=KnowledgeSnapshotFilter(status="processing"),
        )
        await session.commit()

        try:
            paths, temp_dir = await storage_manager.download_files_to_temp_dir(
            ...
            await snapshots_service.update(
                session=session,
                filters=KnowledgeSnapshotFilter(id=snapshot_id),
                values=KnowledgeSnapshotFilter(status="ready", index_id=index_id),
            )
            await session.commit()
            ...
            await snapshots_service.update(
                session=session,
                filters=KnowledgeSnapshotFilter(id=snapshot_id),
                values=KnowledgeSnapshotFilter(status="failed"),
            )
            await session.commit()
            raise
