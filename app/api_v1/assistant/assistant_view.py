...
async def question(
    ...
    assistant = await yandex_service.create_assistant(
        model=yandex_service.model, tools=[index]
    )
    await users_service.update(
        session=session,
        filters=UserFilter(id=user.id),
        values=UserUpdate(assistant_id=assistant.id),
    )
...
async def ask_question(
    ...
    if not assistant_id:
        ...
        await users_service.update(
            session=session,
            filters=UserFilter(id=user.id),
            values=UserUpdate(assistant_id=assistant_id),
        )
    ...
    if not thread_id:
        ...
        await users_service.update(
            session=session,
            filters=UserFilter(id=user.id),
            values=UserUpdate(thread_id=thread_id),
        )
    ...
    if not chat:
        chat = await assistant_chats_service.add(
            session=session, values=AssistantChatCreate(user_id=user.id)
        )
    await assistant_messages_service.add(
        session=session,
        values=AssistantMessageCreate(
    ...
    await assistant_messages_service.add(
        session=session,
        values=AssistantMessageCreate(
            text=ans, assistant_chat_id=chat.id, user_id=user.id, type="assistant"
        ),
    )
